...
void init(int windowSize)  {
    ...
    glEnable(GL_LIGHTING); // Por que ao comentar o objeto muda de cor? Por que perdeu as arestas internas? R: Habilita o cálculo da luz
    glEnable(GL_LIGHT0);
...
void DrawObj(double size) {
    ...
    glPushMatrix();
        ...
        glColor3f(1,0,0); // Para que serve?
        // glRotatef(45, 0,1,0);
        glutSolidCube(size);
    glPopMatrix();

    GLfloat materialEmission2[] = { 00.00, 0.00, 0.00, 1.0};
    GLfloat materialColor2[] = { 0.0, 0.0, 1.0, 1.0 };
    GLfloat mat_specular2[] = { 1.0, 1.0, 1.0, 1.0};
    GLfloat mat_shininess2[] = { 128 };

    glPushMatrix();
        glMaterialfv(GL_FRONT, GL_EMISSION, materialEmission2);
        glMaterialfv(GL_FRONT, GL_AMBIENT, materialColor2);
        glMaterialfv(GL_FRONT, GL_DIFFUSE, materialColor2);
        glMaterialfv(GL_FRONT, GL_SPECULAR, mat_specular2);
        glMaterialfv(GL_FRONT, GL_SHININESS, mat_shininess2);
        // glTranslatef(1, 0, 1);
        // glutSolidSphere(size/2, 20, 10);
        glTranslatef(0, 0, 2);
        glutSolidCube(size);
    glPopMatrix();
}
